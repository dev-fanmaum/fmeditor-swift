...
/// FMEditorDelegate defines callbacks for the delegate of the FMEditorView
@objc public protocol FMEditorDelegate: class {
    /// Called when the inner height of the text being displayed changes
    /// Can be used to update the UI
    @objc optional func fmEditor(_ editor: FMEditorView, heightDidChange height: Int)
    
    /// Called whenever the content inside the view changes
    @objc optional func fmEditor(_ editor: FMEditorView, contentDidChange content: String)
    
    /// Called when the rich editor starts editing
    @objc optional func fmEditorTookFocus(_ editor: FMEditorView)
    
    /// Called when the rich editor stops editing or loses focus
    @objc optional func fmEditorLostFocus(_ editor: FMEditorView)
    
    /// Called when the FMEditorView has become ready to receive input
    /// More concretely, is called when the internal WKWebView loads for the first time, and contentHTML is set
    @objc optional func fmEditorDidLoad(_ editor: FMEditorView)
    ...
    @objc optional func fmEditor(_ editor: FMEditorView, shouldInteractWith url: URL) -> Bool
    
    /// Called when custom actions are called by callbacks in the JS
    /// By default, this method is not used unless called by some custom JS that you add
    @objc optional func fmEditor(_ editor: FMEditorView, handle action: String)
}
...
public class FMEditorWebView: WKWebView {
...
@objcMembers open class FMEditorView: UIView, UIScrollViewDelegate, WKNavigationDelegate, UIGestureRecognizerDelegate {
    ...
    open private(set) var webView: FMEditorWebView
    ...
    open private(set) var contentHTML: String = "" {
        didSet {
            delegate?.fmEditor?(self, contentDidChange: contentHTML)
        }
    }
    ...
    open private(set) var editorHeight: Int = 0 {
        didSet {
            delegate?.fmEditor?(self, heightDidChange: editorHeight)
        }
    }
    ...
    public func setEditorBackgroundColor(_ color: UIColor) {
        runJS("RE.setBackgroundColor('\(color.hexRich)')")
    }
    ...
    public func setTextColor(_ color: UIColor) {
        ...
        runJS("RE.setTextColor('\(color.hexRich)')")
    }
    
    public func setEditorFontColor(_ color: UIColor) {
        runJS("RE.setBaseTextColor('\(color.hexRich)')")
    }
    
    public func setTextBackgroundColor(_ color: UIColor) {
        runJS("RE.prepareInsert()")
        runJS("RE.setTextBackgroundColor('\(color.hexRich)')")
    }
...
//    public func addRowToTable() { runJS("RE.addRowToTable()") }
    ...
    private func performCommand(_ method: String) {
        if method.hasPrefix("ready") {
            ...
            delegate?.fmEditorTookFocus?(self)
        }
        else if method.hasPrefix("blur") {
            delegate?.fmEditorLostFocus?(self)
        }
            ...
            runJS("RE.getHtml()") { content in
                ...
                self.delegate?.fmEditor?(self, handle: action)
            }
        }
    }
